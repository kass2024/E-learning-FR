...
const MaterialsUpload: React.FC<MaterialsUploadProps> = ({
  ...
  const uploadFile = async (file: UploadedFile): Promise<PCloudFile | null> => {
    try {
      setFiles(prev => prev.map(f => 
        f.id === file.id ? { ...f, status: 'uploading', progress: 0 } : f
      ));

      // Simulate progress updates (since pCloud doesn't provide progress)
      ...
      const pcloudFile = await pcloudService.uploadFile(file, courseId || 0);
